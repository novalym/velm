# Path: parser_core/logic_weaver/engine.py
# ----------------------------------------

from pathlib import Path
from typing import List, Dict, Any, Tuple, Optional

from .state.engine import GnosticContext  # <--- THE ASCENDED SANCTUM
from .traversal.engine import TraversalEngine  # <--- THE ASCENDED SANCTUM
...
class GnosticLogicWeaver:
    ...
    4.  **The Unbreakable Vessel:** It forges and holds the ascended `GnosticContext`
        sanctum, protecting the living memory of the alchemical process.
    ...
    def __init__(
            ...
            # [ASCENSION]: The Sacred Quaternity Type Hint
        ...
        # Instantiate the Ascended Directory-based Context
    ...
    def weave(self) -> Tuple[
        List[ScaffoldItem],
        List[Quaternity],  # The Sacred Quaternity
        List[Heresy],
        List[Edict]
    ]:
        """
        ...
        try:
            ...
            # The Ascended TraversalEngine walks the Gnostic Tree natively
            ...
            # 2. KINETIC WILL (Maestro's Edicts - Safe Quaternity)
            ...
            # Inject metadata into the raw dictionary context for downstream scrying
